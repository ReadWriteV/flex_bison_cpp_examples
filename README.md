...
class Scanner : public FlexLexer
{
   ...
   int yylex() override;
   // or custom lex function name and type
...
xxx yyFlexLexer::yyy() { /* code generated by flex */ }
...
The generated C++ parser will call yylex() in the parse member function. This is inconsistent with the C++ interface scanner generated by Flex. To bridge Flex and Bison in C++ interface, you can define the function yylex which forwards return value from the scanner. However this needs yylex can access the scanner instance. To to so, you can use the option `%param {Scanner &scanner}` which will change the yylex and the constructor of parser to accept additional parameter *scanner*.
